...
function tableRows(tables) {
  return tables
    .map((table) => {
      if (table.metadataUnavailable)
        return `<tr><td><a href="${tableHash(table.TableName)}" class="table-link"><span class="table-symbol">${icon("table")}</span><div>${esc(table.TableName)}<small>${esc(table.accessMessage)}</small></div></a></td><td><span class="tag">Details unavailable</span></td><td>—</td><td>—</td><td class="table-secondary-col">—</td><td><a class="icon-button" href="${tableHash(table.TableName)}" aria-label="Browse ${esc(table.TableName)}">${icon("arrow")}</a></td></tr>`;
...
function renderTables() {
  main.innerHTML =
    ...
    activeBanner() +
    `<section class="panel table-picker"><form id="open-table-form" class="panel-body"><div class="field"><label for="open-table-name">Select or enter a table</label><input id="open-table-name" list="available-tables" placeholder="Table name" required pattern="[a-zA-Z0-9_.\\-]{3,255}" autocomplete="off"><datalist id="available-tables">${state.overview.tables.map((t) => `<option value="${esc(t.TableName)}"></option>`).join("")}</datalist><small>Tables belong to the current account and region. Select a suggestion or enter a known name.</small></div><button class="button primary" type="submit">Open table</button></form></section>` +
    tablePanel(state.overview.tables, true);
  document.getElementById("open-table-form").onsubmit = (event) => {
    event.preventDefault();
    location.hash = tableHash(
      document.getElementById("open-table-name").value.trim(),
    );
  };
  document
    .getElementById("table-filter")
