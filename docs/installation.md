...
Version 0.5.0 is a local preview with a bundled Python service. You do not need a
...
On Apple Silicon macOS, open `dynamodb-tools-0.5.0-mac-arm64.dmg`, drag **DynamoDB
...
`dynamodb-tools-0.5.0-darwin-arm64.vsix`, then run **DynamoDB Tools: Open Console**
...
In extension 0.5.0, you can also click the **DynamoDB Tools** database icon in the
...
and cursor selection. Both installations include the Query planner under each table and Copy data in the sidebar. Copy batches remain in service memory across connection changes, but expire after 30 minutes and are discarded when the service stops. Export transformed JSON to retain a batch.
